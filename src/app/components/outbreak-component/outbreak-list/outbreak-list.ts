...
export class OutbreakListComponent implements OnInit {
  ...
  // ── Frontend-only filters (disease & severity not in API) ───
  diseaseFilter = '';
  severityFilter = '';
  ...
  // Apply disease & severity filters on frontend
  applyFrontendFilters(): void {
    this.filteredOutbreaks = this.allOutbreaks.filter(o => {
      const matchDisease  = !this.diseaseFilter  ||
        o.disease.toLowerCase().includes(this.diseaseFilter.toLowerCase());
      const matchSeverity = !this.severityFilter ||
        o.severity.toLowerCase() === this.severityFilter.toLowerCase();
